 ...
 *     styles/cli-flavor.css 的完整内容（作为 JS 模板字符串内联）。
 *   - 把「整行独立占位标记」/*__CLI_FLAVOR_WHALE__*\/ 替换为
 *     lib/whale-data.js 的完整内容（鲸鱼数据，纯 JS 常量声明）。
 *   - 输出 lib/client.js。
 *   - CSS 中若出现反引号、${ 或 \，会做转义处理。
 *   - 替换 WHALE 标记时只匹配「整行只有标记（允许行首缩进）」的占位行，
 *     绝不命中注释里出现的字样，避免把数据嵌进注释行产生非法尾随文本。
 */
...
// （2026-08-28：hero 美术已全部移除，不再内联 whale-data.js。）
const out = tpl
